...
- Added `COMBAT_BATTLEFIELD_LAYOUT_V1` as the finalized battlefield ratio rule based on the S1 Kingdom Border Battlefield Layout Refinement validation image.
- Added `S1_Kingdom_Border_Battlefield_Layout_V1_Reference.png` as a temporary BattleScene coordinate calibration background.
- Added `BattlefieldLayoutReference` under `BattleArea` as a background-layer reference node for manual coordinate calibration.

### Protected

- Did not modify Player Formation, Enemy Formation, Boss, or Summon coordinates.
- Did not modify combat logic, targeting, Skill Queue, Battle Log, bottom UI, damage values, status effects, or runtime combat behavior.
- The reference image is not final runtime art and remains calibration-only.

## 2026-06-16

### Added

- Added `COMBAT_BATTLEFIELD_VISUAL_RULE_V4` as the finalized battlefield visual composition rule.
- Defined Corridor Composition, diagonal confrontation axis, slight opposing formation rotations, Boss-centered enemy formation, horizontal centered Combat Focus Zone, and upper-left Chapter Landmark Zone restrictions.
- Added BattleScene V4 layout validation pass: Chapter Landmark Zone node, V4 default player/enemy slot coordinates, dynamic player formation positions, dynamic enemy-count formation positions, and independent Boss position.
- Temporarily moved the four reserve/support buttons to the battlefield center area and adjusted player formation coordinates lower in the V4 corridor.

### Protected

- During the documentation adoption pass, did not modify Godot scenes, scripts, combat timing, damage values, Skill Queue behavior, Boss behavior, target-line rules, or formation layer rules.
- Did not change combat system logic, targeting, Skill Queue logic, damage logic, status effect logic, Battle/Loot/Story logs, Expedition, Hub Tavern, or World Map behavior during the BattleScene V4 layout validation pass.
- Did not change support-button behavior, summon logic, targeting, combat pacing, damage, status effects, or logs while moving the reserve/support buttons.
